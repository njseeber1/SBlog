...
function fotografie_theme_options( $wp_customize ) {
    ...
    $wp_customize->add_setting( 'fotografie_disable_static_page_posts', array(
        'sanitize_callback' => 'fotografie_sanitize_checkbox',
    ) );

    $wp_customize->add_control( 'fotografie_disable_static_page_posts', array(
        'label'           => esc_html__( 'Check to disable Recent Posts on Static Frontpage', 'fotografie' ),
        'section'         => 'fotografie_homepage_options',
        'type'            => 'checkbox',
    ) );
	...
	*/
	...
	// For WooCommerce layout: fotografie_woocommerce_layout, check woocommerce-options.php.
	/* Scrollup Options */
